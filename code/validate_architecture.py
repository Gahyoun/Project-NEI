...
def main() -> int:
    ...
    if validation.get("status") != "bounded corrected study complete · full-corpus validation pending":
        raise ValueError("unreviewed promotion of the bounded empirical validation status")
    corrected = read("corrected-study")
    audit = corrected.get("artifact_audit", {})
    if audit.get("status") != "pass" or audit.get("config_sha256") != corrected.get("config_sha256"):
        raise ValueError("corrected result lacks matching independent artifact audit")
    if corrected.get("counts") != {"graphs": 330, "attempted_runs": 33000, "terminal_optimizations": 40000}:
        raise ValueError("unexpected bounded-study execution counts")
